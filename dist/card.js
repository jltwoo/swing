    ...
    var construct = function construct() {
        ...
        eventEmitter.on('panend', function (e) {
            ...
            if (config.isThrowOut(x, targetElement, config.throwOutConfidence(x, targetElement))) {
                console.log(x, y);
                var pileCoord = x < 0 ? config.leftPileCoord : config.rightPileCoord;
                if (pileCoord) {
...
Card.transform = function (element, x, y, r) {
    (0, _raf2.default)(function () {
        var tz = element.style[(0, _vendorPrefix2.default)('transform')];
        var match = tz.match(/translate3d\(\-?\d+px\,\s*\-?\d+px\,\s*\-?\d+px\)/gi);

        var _ref = match.length ? match[0].match(/\-?\d+px/gi) : ['0px', '0px', '0px'];
        ...
        element.style[(0, _vendorPrefix2.default)('transform')] = 'translate3d(' + Math.round(x) + 'px, ' + Math.round(y) + 'px, ' + z_px + ') rotate(' + r + 'deg)';
    });
};

Card.recalculateTranslateZ = function (targetElement, resetStack) {
    ...
    (0, _raf2.default)(function () {
        for (var i = 0; i < targetElement.parentNode.children.length; i++) {
            var tz = targetElement.parentNode.children[i].style[(0, _vendorPrefix2.default)('transform')];
            var match = tz.match(/translate3d\(\-?\d+px\,\s*\-?\d+px\,\s*\-?\d+px\)/gi);

            var _ref3 = match.length ? match[0].match(/\-?\d+/gi) : [null, '0', '0', '0'];
            ...
            targetElement.parentNode.children[i].style[(0, _vendorPrefix2.default)('transform')] = tz.replace(/translate3d\(\-?\d+px\,\s*\-?\d+px\,\s*\-?\d+px\)/gi, 'translate3d(' + x + 'px, ' + y + 'px, ' + z + 'px)');
        };
    });
};
